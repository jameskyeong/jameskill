...
**Develop**:
The development orchestrator skill (`/jsk:develop`). Takes raw requirements through a disciplined pipeline — clarify → route → build → review → verify → finish — to produce production-grade code. The router branches 4-way: **DIRECT** / **PLAN** for feature work, **DIAGNOSE** for bugs, **PROTOTYPE** for throwaway exploration. Each phase hardens the work one pass at a time — no phase boundary is crossed on soft language.
_Avoid_: workflow (ambiguous — see Flagged Ambiguities), temper (v1 name, retired), forge (v2.0 name, retired), powertasking (alpha.11–16 name, retired — ADR 0015)

**Production-readiness gate**:
The architectural concept spanning the jameskill v2.0 pipeline: **develop** builds the code, an **inspection layer** (currently `jsk:security` alone) checks service-configuration security, **ship** issues the final verdict. Not a single skill — the emergent property of these working in sequence. v2.0 ships the inspection layer as security-only; multi-axis inspection (design / quality / performance / dependencies) is post-v2.0 work, deliberately not promised — see [ADR 0010](docs/adr/0010-launch-v0.1-security-only.md).
...
A discipline enforced at every phase boundary within **develop**: no phase may declare completion without running verification commands and directly observing the output. "Should work" and "seems to pass" are treated as "not verified."
_Avoid_: check, validation (too vague)

**Discipline references**:
The modular library at `skills/develop/references/`. Each module deepens one develop phase (or one cross-cutting concern) — principles, anti-patterns, edge cases — so develop's depth does not depend on external skills. `SKILL.md` is the orchestrator; each phase carries a pointer to its reference. See [ADR 0005](docs/adr/0005-forge-depth-references.md) for the rationale.
_Avoid_: helpers, support files (too vague — these are load-bearing discipline)

**Retrospective**:
The develop phase that runs between **Verify** and **Finish** to deposit this session's learnings into the repo's compound-engineering channels. Checks four channels in order — ADR (`docs/adr/`), discipline references (`skills/develop/references/`), CONTEXT.md domain glossary (final sweep — terms deposit inline when they resolve), `.out-of-scope/` rejection records — and proposes a deposit only when the channel's threshold is met. Silent exit is the expected outcome for routine small changes; the phase is successful when it correctly identifies that no deposit is needed. See [ADR 0007](docs/adr/0007-retrospective-phase.md) and [`references/retrospective.md`](skills/develop/references/retrospective.md).
...
The architectural principle — the second of jameskill's two pillars alongside the **develop** orchestrator — that each session must deposit durable artifacts in the repo so future sessions start ahead. The six compounding channels and how they fill: **plan files** in `docs/plans/<github-id>/` (auto, PLAN route), **DIAGNOSE regression tests** in the test suite (auto, DIAGNOSE route), **ADRs** in `docs/adr/` (auto-prompted by Retrospective, Channel 1), **discipline references** in `skills/develop/references/` (auto-prompted by Retrospective, Channel 2), **CONTEXT.md** domain glossary (inline the moment a term resolves; Retrospective Channel 3 sweeps stragglers), **`.out-of-scope/` rejection records** (auto-prompted by Retrospective, Channel 4).
...
**DIRECT**:
A **develop** routing decision for small, contained changes (1-4 commits, tightly-grouped files). Skips plan file creation and proceeds straight to build.
_Avoid_: simple, small (subjective)

**PLAN**:
A **develop** routing decision for medium-scope features (5-15 commits, 2-4 files with shared state). Produces a plan file in `docs/plans/<github-id>/` before sequential task execution.
_Avoid_: roadmap, spec (different things)

**DIAGNOSE**:
A **develop** routing decision for bug-first work. Five steps: Reproduce (write a test that fails because of the bug) → Minimize (trim until the test exercises only the bug-causing surface) → Investigate (hypothesis-driven code reading) → Fix (standard GREEN discipline) → Regression-prevent (the minimized test stays in the suite permanently). The minimized reproduction test is the regression net. See [ADR 0006](docs/adr/0006-forge-route-expansion.md).
_Avoid_: debug, troubleshoot (too vague — DIAGNOSE is the route name with a specific phase order)

**PROTOTYPE**:
A **develop** routing decision for throwaway exploration ("which design fits", "try a few approaches"). TDD is intentionally relaxed; two or three meaningfully different variations are built within a time-box; the user reviews and chooses Discard (default), Promote-to-Plan (kicks off a fresh PLAN run on the chosen variation), or Re-Clarify (the question was wrong). Prototype code does not graduate to production — Promote means restart with full discipline. See [ADR 0006](docs/adr/0006-forge-route-expansion.md).
_Avoid_: spike (too informal — PROTOTYPE has explicit phase order and exit gate), POC (overloaded term)

### Inspection

**Security**:
Alpha utility (`/jsk:security`) for service-configuration security inspection — Supabase RLS gaps, secret-key client exposure, missing Stripe webhook signatures. v0.1 ships in v2.0 but is **off the marketing surface** (see ADR 0004): the author does not rely on it daily, and three live company codebases returned zero BLOCK findings during dogfooding. Reachable for users who want it; future direction (wrap, deep, or sunset) is deferred to v2.1+. Named for the lockdown step before a mech sortie.
_Avoid_: harden (v1 name, retired), lock (v2.0 name, retired), production-readiness gate (security is not the gate in v2.0; see ADR 0004), security-check (alpha.13–16 name, retired — ADR 0015).

### Ship ecosystem

**Ship**:
Alpha utility (`/jsk:ship`) that issues a **security** GO / NO-GO verdict from `security`'s findings. v0.1 is deliberately security-only — there is no design / quality / performance axis, and multi-axis aggregation is not promised (see [ADR 0010](docs/adr/0010-launch-v0.1-security-only.md)). Ships in v2.0 but is **off the marketing surface** for the same reason as security (see ADR 0004). The output reads "LAUNCH READY?". Future direction tied to whether post-launch users ask for it.
_Avoid_: ship-ready (v1 name, retired), launch (v2.0 name, retired), production-readiness gate (deferred from v2.0 surface), multi-axis aggregator (overclaims v0.1 — that framing was retired in ADR 0010), ship-check (alpha.13–16 name, retired — ADR 0015).

### Issue tracking

**Tracker**:
The tracker uplink skill (`/jsk:tracker`). One-time Notion connection — API key, database, property mapping, defaults.
_Avoid_: setup-issue (v1 name, retired), link (v2.0 name, retired), tracker-setup (alpha.13–16 name, retired — ADR 0015)

**Report**:
The issue flagging skill (`/jsk:report`). Parses a prompt into Notion issues, auto-groups related items, verifies against the codebase, creates pages with proper template blocks.
_Avoid_: tag (v2.0 name, retired), report-issue (alpha.13–15 name, retired — ADR 0014), tracker-report (alpha.16 name, retired — ADR 0015)
...
> **Dev**: "I want to wire security into the develop pipeline."
> **Domain expert**: "Develop has a Ship slot that activates when inspection skills are available — that's the wiring point. But for v2.0 the slot stays inert: develop skips it with a note, and `security` runs standalone."
> **Dev**: "So how does ship fit in?"
> **Domain expert**: "Ship is a separate skill the user invokes when they want a deploy verdict. It calls `security`, reads the findings, and issues a security GO / NO-GO. It is not a multi-axis aggregator — that framing was retired in [ADR 0010](docs/adr/0010-launch-v0.1-security-only.md). v0.1 is security-only by design, and post-v0.1 expansion is gated on real user demand, not pre-announced."
