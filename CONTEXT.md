...
The architectural principle — the second of jameskill's two pillars alongside the **develop** orchestrator — that each session must deposit durable artifacts in the repo so future sessions start ahead. The five compounding channels and how they fill: **plan files** in `docs/plans/<github-id>/` (auto, PLAN route), **DIAGNOSE regression tests** in the test suite (auto, DIAGNOSE route), **ADRs** in `docs/adr/` (auto-prompted by Retrospective phase, Channel 1), **discipline references** in `skills/develop/references/` (auto-prompted by Retrospective, Channel 2), **CONTEXT.md** domain glossary (auto-prompted by Retrospective, Channel 3).
