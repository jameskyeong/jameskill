...
**Tracker-free.** This workflow has no external dependencies beyond git. All artifacts (specs, plans, PRDs, ADRs, CONTEXT.md) live in the repository. Cross-session work resumes by re-running `/jsk:workflow-external` with the relevant plan or PRD file path as context.
...
>
> Re-run `/jsk:workflow-external` after installation completes.
...
- If invoked from `jsk:strike`, use the issue title + body as the starting context
...
If the session ends mid-execution, the user resumes by running `/jsk:workflow-external` with the plan file path as context. Phase 0 (grill-me) is skipped when the plan file already encodes the agreed problem statement. Re-invoking `subagent-driven-development` on the same plan picks up from incomplete tasks.
...
| Out of scope, large | Note for the user — suggest `/jsk:tag` to track separately in Notion |
...
| **Minor** | Note in the Phase 5.2 commit body, suggest `/jsk:tag` for follow-up |
...
| Low / Info | Note in the Phase 5.2 commit body, suggest `/jsk:tag` for follow-up |
...
When invoked directly via `/jsk:workflow-external`:
...
- The user can resume by running `/jsk:workflow-external` again with the same context (or with the PRD / plan file path for Routes PRD / PLAN)
