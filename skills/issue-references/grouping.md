...
Canonical auto-grouping algorithm used by `/report` (during issue parsing) and `/resolve` (during multi-issue selection). Both skills MUST follow this algorithm so grouping behavior is consistent.

The goal: cluster issues that genuinely belong together so a single develop session (or a single Notion issue) addresses related problems with shared context. Over-grouping pollutes the result with unrelated work; under-grouping misses obvious affinities.
...
- **Code-file overlap** — the two issues touch overlapping files or modules in the codebase. `/report`'s Codebase verification step surfaces this directly; `/resolve` infers it from issue bodies when available.
...
Before either skill *commits* to the grouping (writing Notion pages in `/report`, invoking `/develop` in `/resolve`), the proposed groups MUST be shown to the user with a one-line rationale per pair link. Users override by replying with a correction.
...
### Cross-status grouping in /resolve

`/resolve` should not propose grouping a "pending" issue with an "in progress" issue. The user picked them from different lists; treating them as a unit collapses meaningful state distinction.

**Cure**: `/resolve` partitions selections by status before running the grouping algorithm. Run grouping within each status partition, present them as separate group sets.
...
**Cure**: if `/report`'s codebase verification reveals "already in progress on branch X" for one item but the other is fresh, downgrade the code-file overlap signal for that pair. Skills should annotate this in the rationale ("would group on auth/* but item A is already shipping on feat/auth-redesign").
