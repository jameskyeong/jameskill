...
Canonical auto-grouping algorithm used by `/tracker-report` (during issue parsing) and `/tracker-resolve` (during multi-issue selection). Both skills MUST follow this algorithm so grouping behavior is consistent.
...
- **Code-file overlap** — the two issues touch overlapping files or modules in the codebase. `/tracker-report`'s Codebase verification step surfaces this directly; `/tracker-resolve` infers it from issue bodies when available.
...
Before either skill *commits* to the grouping (writing Notion pages in `/tracker-report`, invoking `/powertasking` in `/tracker-resolve`), the proposed groups MUST be shown to the user with a one-line rationale per pair link. Users override by replying with a correction.
...
### Cross-status grouping in /tracker-resolve

`/tracker-resolve` should not propose grouping a "pending" issue with an "in progress" issue. The user picked them from different lists; treating them as a unit collapses meaningful state distinction.

**Cure**: `/tracker-resolve` partitions selections by status before running the grouping algorithm. Run grouping within each status partition, present them as separate group sets.
...
**Cure**: if `/tracker-report`'s codebase verification reveals "already in progress on branch X" for one item but the other is fresh, downgrade the code-file overlap signal for that pair. Skills should annotate this in the rationale ("would group on auth/* but item A is already shipping on feat/auth-redesign").
