...
**v1 limit**: this does NOT invoke `/develop` headlessly. It checks the skill files themselves + hand-written fixture samples representing ideal develop output. Real session evals (running `claude -p` against fixture prompts) are v2.
...
│   ├── no-banned-phrases.mjs       ← develop verification banned soft-language
...
│   └── cross-cutting-gates-present.mjs ← skills declare/inherit develop gates
...
2. Make the body realistic — a real shape of develop output, not synthetic test data.
...
A "real" eval would call `claude -p "<prompt>"` against each fixture, capture develop's output, and apply the same checks. We deliberately deferred that to v2:
